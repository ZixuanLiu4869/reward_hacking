...
def sacred_config(_log):
    ...
    if original_config["env_config"].get("flow_params") is not None:
        ...
        original_reward_fun = "true"
        
        original_reward_spec["true"][0]=('commute',1)
        original_reward_spec["true"][1]=("accel",1)
        original_reward_spec["true"][2]=("headway",0.1)
    ...
    for sample_batch in sample_batches:
        ...
        try:
            ...
            for info in info_list:
                try:
                    rewards_list.append(info["proxy_reward"])
                    commute_list.append(info["commute"])
                    accel_list.append(info["accel"])
                    headway_list.append(info["headway"])
                except Exception as e:
                    print(e)
                    rewards_list.append(0)
            base_trajectories.append({"obs": obs_list.tolist(), "actions": actions_list.tolist(), "rewards": rewards_list, "commute":commute_list, "accel": accel_list, "headway": headway_list})
        except Exception as e:
            ...
            for info in info_list:
                try:
                    ...
                    print(e)
                    rewards_list.append(0)
            orpo_trajectories.append({"obs": obs_list.tolist(), "actions": actions_list.tolist(), "rewards": rewards_list, "commute":commute_list, "accel": accel_list, "headway": headway_list})
...
def main(
    ...
    for policy_id in policy_ids:
        ...
        #print(batches[0][policy_id]["obs"])
        
        #print(batches[0][policy_id]["actions"])
        
        save_trajectories_to_json(all_batches, "base_traffic_samples.json", "orpo_traffic_samples.json")
