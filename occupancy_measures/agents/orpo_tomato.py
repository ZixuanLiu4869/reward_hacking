...
class ORPO(PPO):
    ...
    def calculate_discounted_occupancy(self, trajectories, gamma=0.99):
        """
        Calculate discounted occupancy measures and record immediate rewards for state-action pairs.

        Args:
            trajectories (list): List of trajectories, each containing 'obs', 'actions', and 'rewards'.
            gamma (float): Discount factor for weighting future timesteps.

        Returns:
            dict: A dictionary where keys are (state, action) pairs and values are tuples:
                (discounted occupancy measure, list of rewards for each occurrence).
        """
        state_action_counts = defaultdict(float)

        # Iterate through all trajectories
        for trajectory in trajectories:
            obs = trajectory[SampleBatch.OBS]         # List of states (string format)
            actions = trajectory[SampleBatch.ACTIONS] # List of actions

            # Track the discount factor
            discount = 1.0
            
            # Record each state-action pair with discounted weighting
            for state, action in zip(obs, actions):
                # Convert state to a tuple of integers for hashability
                state_tuple = tuple(int(x) for x in state)

                formatted_key = self.format_key((state_tuple, action))
                
                state_action_counts[formatted_key] += discount
                
                discount *= gamma  # Apply discount factor for the next step

        # Normalize to calculate occupancy measures
        total_weight = sum(state_action_counts.values())

        occupancy_measure= {k: v / total_weight for k, v in state_action_counts.items()}

        return occupancy_measure
    
    def get_occ_lookup_table(self, file_path):
        with open(file_path, "r") as f:
            return json.load(f)

    def format_key(self, key):
        """Convert (state, action) key into structured JSON-like representation."""
        state, action = key
        # Separate agent and tomatoes
        agent_state = state[:-10]  # First part of the state (excluding last 10 values)
        tomatoes_state = state[-10:]  # Last 10 values of the state
        
        # Find the agent's position (index of `1` in the first part)
        agent = agent_state.index(1) if 1 in agent_state else -1

        formatted_state = (
            ("agent", agent),
            ("tomatoes", tomatoes_state)
        )
        return str((formatted_state, action))
    
    def compute_kai_distance(self, base_occupancy_measure, occupancy_measure):
        distance = 0

        for key, base_occ in base_occupancy_measure.items():
            try:
                occ = occupancy_measure[key]
            except Exception as e:
                occ = 0
            distance += occ*occ/base_occ[1]
        return distance -1
    
    def _min_max_discriminator_rewards(self, info, safe_policy_id, chi2_rewards, proxy_reward, mean_reward, square_reward, kai_distance):
        #chi2_rewards_sorted = np.sort(chi2_rewards)
        #i = int(0.01 * len(chi2_rewards_sorted))
        #occupancy_measure_chi2_trimmed = np.mean(chi2_rewards_sorted[i:-i])
        #info[safe_policy_id][
        ...
        if kai_distance <= 0:
            rewards = (chi2_rewards-mean_reward*proxy_reward)
        elif kai_distance - square_reward < 0:
            rewards = (chi2_rewards-mean_reward*proxy_reward) / np.sqrt(kai_distance)
        else:
            rewards = (chi2_rewards-mean_reward*proxy_reward) / np.sqrt(kai_distance-square_reward)
        return rewards
    ...
    def augment_reward_with_discriminator_and_transfer_action_dist_inputs(
        ...
        for policy_id in self.config.safe_policy_ids:
            if policy_id in train_batch.policy_batches:
                ...
                occupancy_measure_current = self.calculate_discounted_occupancy(episode_batches, gamma)

                base_occ_table = self.get_occ_lookup_table("normalized_reward_data.json")
                ...
                #print(mean_reward_safe)
                #print(sqrt_var_mean_reward_safe)


                #episode_lens = len(episode_batches)
                #mid = int(episode_lens/2)
                #square_reward = np.mean(episode_rewards[:mid])*(1-gamma)*np.mean(episode_rewards[mid:])*(1-gamma)
                ...
                obs_list = current_batch[SampleBatch.OBS]
                actions_list = current_batch[SampleBatch.ACTIONS]

                # Convert observations into hashable tuples
                state_action_pairs = [self.format_key((tuple(int(x) for x in obs), action)) for obs, action in zip(obs_list, actions_list)]

                # Compute the occupancy ratio, ensuring a safe division (avoid divide by zero)
                occupancy_ratios = np.zeros(len(state_action_pairs))
                
                
                num_mismatch = 0
                occu_mismatch = 0
                mismatch_state_actions = []
                for i, key in enumerate(state_action_pairs):
                    occ_current = occupancy_measure_current.get(key, 0)
                    occ_base = base_occ_table.get(key, None)  # Small value to avoid div by zero
                    if occ_base is None or occ_base[1] == 0:  
                        # If base_occ_table doesn't have this key or value is zero, set a large ratio
                        occupancy_ratios[i] = 1e6
                        num_mismatch += 1
                        occu_mismatch += occ_current
                        mismatch_state_actions.append(key)
                    else:
                        occupancy_ratios[i] = occ_current / occ_base[1]
                   
                print(num_mismatch)
                print(occu_mismatch)

                info[self.config.current_policy_id]["num_mismatch"] = num_mismatch
                info[self.config.current_policy_id]["occ_mismatch"] = occu_mismatch
                info[self.config.current_policy_id]["mismatch_state_action"] = "|".join(mismatch_state_actions)

                kai_distance = self.compute_kai_distance(base_occ_table, occupancy_measure_current)
                '''
                dicriminator_occupancy_ratios=self._chi2_discriminator_rewards(info, policy_id, discriminator_policy_scores)
                
                # X-axis values (index positions)
                x = np.arange(len(occupancy_ratios[:100]))
                
                
                # Plot both lists
                plt.plot(x, occupancy_ratios[:100], marker='o', label='sample', linestyle='-')
                plt.plot(x, dicriminator_occupancy_ratios[:100], marker='s', label='orpo', linestyle='--')

                # Add labels and title
                plt.xlabel("Index")
                plt.ylabel("Values")
                plt.title("Comparison of Two Occupancy Measure")
                plt.legend()
                plt.grid(True)
                ...
                #distance = self._sqrt_chi2_kai_rewards(info, policy_id, occupancy_ratios, kai_distance)
                #if occupancy_measure_divergence_coeff != 0:
                    #distance[~np.isfinite(distance)] = 0
                    #per_policy_discriminator_reward += (
                        #-occupancy_measure_divergence_coeff * distance
                ...
                distance = self._min_max_discriminator_rewards(info, policy_id, occupancy_ratios, current_batch[SampleBatch.REWARDS], mean_reward, square_reward, kai_distance)
                correlated_r = 0.8
                ...
                info[self.config.current_policy_id]["mean_square_reward"] = square_reward
                info[self.config.current_policy_id]["kai_distance"] = kai_distance
                print(kai_distance)

                #normalized_reward = (current_batch[SampleBatch.REWARDS]-mean_reward_safe)/sqrt_var_mean_reward_safe
    ...
    def training_step(self) -> ResultDict:
        ...
        #plt.title('Discriminator Loss For one epoch')
