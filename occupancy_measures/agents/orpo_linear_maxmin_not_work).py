...
class ORPO(PPO):
    ...
    def weighted_orthonormalize(self, B_array, weights):
        """
        Given B (N x M) and weights (N,), transform B to B_tilde such that:
            B_tilde.T @ B_tilde = I
        where B_tilde = W_sqrt @ B @ R^{-1}
        and W_sqrt is the diagonal sqrt(weights)

        Args:
            B_array (np.ndarray): (N, M) matrix of features
            weights (np.ndarray): (N,) vector of positive weights

        Returns:
            B_tilde (np.ndarray): Transformed (N, M) matrix satisfying B_tilde.T @ B_tilde = I
        """
    ...
    def caculate_linear_worse_reward(
        ...
        current_batch_copy = current_batch.copy()
        episode_batches = current_batch_copy.split_by_episode()
        current_batch_copy_ = current_batch.copy()
        current_batch_copy_[SampleBatch.REWARDS] = chi2_rewards_reverse   # record the pi_ref/pi
        episode_batches_ = current_batch_copy_.split_by_episode()
        for i in range(len(episode_batches)):
            info_list = episode_batches[i][SampleBatch.INFOS]
            vel = np.array([entry["vel"] for entry in info_list[1:] if "vel" in entry])
            accel = np.array([entry["accel"] for entry in info_list[1:] if "vel" in entry])
            headway = np.array([entry["headway"] for entry in info_list[1:] if "vel" in entry])
            vel_list.extend(vel)
            accel_list.extend(accel)
            headway_list.extend(headway)
            reward_list = (episode_batches[i][SampleBatch.REWARDS]-mean_reward)/sqrt_var_mean_reward
            if len(vel) != len(reward_list[:-1]):
                continue
                #print(info_list)
                #print(episode_batches_safe[i][SampleBatch.REWARDS])
            normalized_reward.extend(reward_list[:-1])
            weight_list.extend(episode_batches_[i][SampleBatch.REWARDS][:-1])
        ...
        B_orthogonal = self.weighted_orthonormalize(B_array, weight)
        ...
        # solve lambda
        lambdas = self.solve_lambda(v, d, c, correlated_r)
        ...
        for i in range(len(episode_batches)):
            ...
            reward_list = (episode_batches[i][SampleBatch.REWARDS]-mean_reward)/sqrt_var_mean_reward
            if len(vel) != len(reward_list[:-1]):
                final_reward.extend(reward_list)
                continue
                #print(info_list)
                #print(episode_batches_safe[i][SampleBatch.REWARDS])
            length = len(vel)
            final_reward.extend(linear_max_min_reward[count:count+length])
            count += length
            final_reward.append(reward_list[-1])

        return final_reward
        
        
    def solve_lambda(self, v, d, c, r, init_lambda=None):
        """
        ...
        if init_lambda is None:
            init_lambda = np.array([1.0, 1.0, -0.1])

        def grad_fn(lam):
            lam1, lam2, lam3 = lam
            u = v - lam1 * d - lam2 * c
            
            grad1 = -0.5 * lam3 * d.T @ u + r
            grad2 = -0.5 * lam3 * c.T @ u
            grad3 = -0.25 * lam3**2 * u.T  @ u + 1
        ...
        try:
            result = root(grad_fn, init_lambda, method='lm')
            #result = least_squares(grad_fn, init_lambda, method="lm", max_nfev=4000)
    ...
    def training_step(self) -> ResultDict:
        ...
        if ORPO_DISC_TRAINING or LINEAR_MAX_MIN or MAX_MIN:
            ...
            # Change learning rate, traffic: 5e-3, 200 step; 
