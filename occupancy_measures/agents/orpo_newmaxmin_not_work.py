...
class ORPO(PPO):
    ...
    def caculate_linear_worse_reward(
            self, info, current_batch, train_patch, policy_id, discriminator_policy_scores
    ):
        discounted_sum = lambda rewards, gamma: np.sum(np.array(rewards) * np.power(gamma, np.arange(len(rewards))))
        gamma = 0.99
    ...
    def solve_lambda(self, v, d, c, r, init_lambda=None, lambda3_epsilon=1e-2):
        """
    ...
    def training_step(self) -> ResultDict:
        ...
        if ORPO_DISC_TRAINING or LINEAR_MAX_MIN or MAX_MIN:
            ...
            # Change learning rate, traffic: 5e-3, 200 step; pandemic: 1e-3, 50. 
