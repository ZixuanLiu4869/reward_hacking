...
class ORPO(PPO):
    ...
    def _sqrt_chi2_kai_rewards(
        self, info, safe_policy_id, discriminator_policy_scores, proxy_reward, mean_reward, square_reward
    ):
        chi2_rewards = self._chi2_discriminator_rewards(
            info, safe_policy_id, discriminator_policy_scores
        )
        ...
        #print(chi2_rewards)
        #print(mean_reward * proxy_reward)
        if occupancy_measure_chi2_trimmed <= 0:
            rewards = chi2_rewards - mean_reward * proxy_reward
        elif occupancy_measure_chi2_trimmed - square_reward < 0:
            rewards = (chi2_rewards - mean_reward * proxy_reward) / np.sqrt(occupancy_measure_chi2_trimmed)
        else:
            rewards = (chi2_rewards - mean_reward * proxy_reward) / np.sqrt(occupancy_measure_chi2_trimmed-square_reward)

        return rewards
    
    def caculate_worse_reward(
        self, info, current_batch, train_batch, policy_id, discriminator_policy_scores
    ):
        gamma = 0.99
        ...
        chi2_rewards_reverse = 1/(chi2_rewards+1e-6) # this is pi_ref/pi
        
        # Caculate the mean and variance under pi_ref using importance sampling
        current_batch_copy = current_batch.copy()
        current_batch_copy[SampleBatch.REWARDS] = current_batch_copy[SampleBatch.REWARDS] * chi2_rewards_reverse
        episode_batches = current_batch_copy.split_by_episode()
        episode_rewards = []
        for i in range(len(episode_batches)):
            episode_rewards.append(discounted_sum(episode_batches[i][SampleBatch.REWARDS], gamma))
        ...
        normalized_reward = (current_batch[SampleBatch.REWARDS]-mean_reward)/sqrt_var_mean_reward
        assert np.all(np.isfinite(normalized_reward))

        # Caculate mean square reward
        current_batch_copy = current_batch.copy()
        current_batch_copy[SampleBatch.REWARDS] = normalized_reward
        ...
        info[self.config.current_policy_id]["mean_reward"] = mean_reward
        square_reward = mean_reward**2
        print("mean_square_reward", square_reward)
        info[self.config.current_policy_id]["mean_square_reward"] = square_reward
        
        distance = self._sqrt_chi2_kai_rewards(info, policy_id, discriminator_policy_scores, normalized_reward,  mean_reward, square_reward)

        correlated_r = 0.9999 # 1-1e-6 for traffic, 
        #correlated_r = 0.9999 # for glucose
        coeff = np.sqrt(1-correlated_r**2)/correlated_r
        print(coeff)

        return normalized_reward, distance, coeff
    
    def weighted_orthonormalize(self, B_array, weights):
    ...
    def caculate_linear_worse_reward(
        ...
        # Normalize the proxy reward
        #chi2_rewards_reverse = np.maximum(1/(chi2_rewards+1e-6), 1e-2) # this is pi_ref/pi
        ...
        safe_headway_list = []
        for i in range(len(episode_batches_safe)):
            info_list = episode_batches_safe[i][SampleBatch.INFOS]
            safe_vel = np.array([entry["vel"] for entry in info_list[1:] if "vel" in entry])
            safe_accel = np.array([entry["accel"] for entry in info_list[1:] if "vel" in entry])
            safe_headway = np.array([entry["headway"] for entry in info_list[1:] if "vel" in entry])
            safe_vel_list.extend(safe_vel)
            safe_accel_list.extend(safe_accel)
            safe_headway_list.extend(safe_headway)
            reward_list = (episode_batches_safe[i][SampleBatch.REWARDS]-mean_reward_safe)/sqrt_var_mean_reward_safe
            if len(safe_vel) != len(reward_list[:-1]):
                continue
                #print(info_list)
                #print(episode_batches_safe[i][SampleBatch.REWARDS])
            normalized_reward.extend(reward_list[:-1])
        
        # Caculate the parameters of minmax problem
        current_info_batch = current_batch[SampleBatch.INFOS]
        current_vel_list = [entry["vel"] for entry in current_info_batch if "vel" in entry]
        current_accel_list = [entry["accel"] for entry in current_info_batch if "accel" in entry]
        current_headway_list = [entry["headway"] for entry in current_info_batch if "headway" in entry]
        ...
        B_array_current = np.column_stack((current_vel_list, current_accel_list, current_headway_list))  # Shape: (N, 3)

        # Step 2: Compute M = (1/N) * sum_x B(x) B(x)^T = (B^T @ B) / N
        Q = B_array.T @ B_array / len(B_array)  # Shape: (3, 3)

        # Caculate v,d,c, shape (3,)
        v = np.mean(B_array_current, axis=0)  # Shape: (3,)
        print(v)

        d = np.sum(np.array(normalized_reward)[:, np.newaxis]*B_array, axis=0) / len(B_array)  # Shape: (3, )
        print(d)

        c = np.mean(B_array, axis=0) # Shape: (3,)
        print(c)

        # solve lambda
        lambdas = self.solve_lambda(v, d, c, Q, correlated_r)
        ...
        min_max_reward = []
        for i in range(len(episode_batches)):
            info_list = episode_batches[i][SampleBatch.INFOS]
            safe_vel = np.array([entry["vel"] for entry in info_list[1:] if "vel" in entry])
            safe_accel = np.array([entry["accel"] for entry in info_list[1:] if "accel" in entry])
            safe_headway = np.array([entry["headway"] for entry in info_list[1:] if "headway" in entry])
            reward_list = ((theta[0] * safe_vel + theta[1] * safe_accel + theta[2] * safe_headway)).tolist()
            reward_last = episode_batches[i][SampleBatch.REWARDS][-1]
            min_max_reward.extend(reward_list)
            min_max_reward.append(reward_last)

        return min_max_reward
        
        
    def solve_lambda(self, v, d, c, Q, r, init_lambda=None):
        """
        Solve for lambda_1, lambda_2, lambda_3 given v, d, c, Q (3x3 matrix), and scalar r.

        Returns:
            np.array([lambda_1, lambda_2, lambda_3])
        """
        if init_lambda is None:
            init_lambda = np.array([0.0, 0.0, -0.1])
        ...
        def grad_fn(lam):
            ...
            grad1 = -0.5 * lam3 * d.T @ Q_inv @ u + r
            grad2 = -0.5 * lam3 * c.T @ Q_inv @ u
            grad3 = -0.25 * (lam3**2) * u.T @ Q_inv @ u + 1

            return np.array([grad1, grad2, grad3])

        # Use scipy's root-finding to solve for stationary point
        #result = root(grad_fn, init_lambda, method='lm')
        result = least_squares(grad_fn, init_lambda, method="lm", max_nfev=2000)

        if result.success:
            return result.x
        else:
            print("error for solving lambda", result.message)
            return None
        

    def augment_reward_with_discriminator_and_transfer_action_dist_inputs(
        self, train_batch: MultiAgentBatch, ORPO_TRAINING, ORPO_DISC_TRAINING, LINEAR_MAX_MIN, MAX_MIN
        ...
        for policy_id in self.config.safe_policy_ids:
            if policy_id in train_batch.policy_batches:
                ...
                if ORPO_TRAINING or ORPO_DISC_TRAINING:
                    ...
                    if ORPO_DISC_TRAINING:
                        # for caculating the worse reward
                        normalized_reward, distance, coeff = self.caculate_worse_reward(info, current_batch, train_batch, policy_id, discriminator_policy_scores)
                        distance[~np.isfinite(distance)] = 0
                        worse_policy_discriminator_reward = -coeff * distance
                elif MAX_MIN:
                    normalized_reward, distance, coeff = self.caculate_worse_reward(info, current_batch, train_batch, policy_id, discriminator_policy_scores)
                    if coeff != 0:
                        distance[~np.isfinite(distance)] = 0
                    ...
                    min_max_reward = self.caculate_linear_worse_reward(info, current_batch, train_batch, policy_id, discriminator_policy_scores)
                    # for caculating the worse reward
                    normalized_reward, distance, coeff = self.caculate_worse_reward(info, current_batch, train_batch, policy_id, discriminator_policy_scores)
                    distance[~np.isfinite(distance)] = 0
                    worse_policy_discriminator_reward = -coeff * distance
                    ...
                    logger.error(
                            "Must be ORPO, ORPO_DISC or MAX MIN."
                        )
        ...
        if ORPO_TRAINING or ORPO_DISC_TRAINING:
            ...
            current_batch[SampleBatch.REWARDS] = (
                normalized_reward + current_batch[DISCRIMINATOR_REWARDS]
            )
        elif LINEAR_MAX_MIN:
            # Solve weird buggs: delta_t = rewards + gamma * vpred_t[1:] - vpred_t[:-1]
            #ValueError: operands could not be broadcast together with shapes (20000,) (300,)
            current_batch[UNCLIPPED_DISCRIMINATOR_REWARDS] = current_batch[DISCRIMINATOR_REWARDS] = np.zeros_like(current_batch[SampleBatch.REWARDS])

            current_batch[SampleBatch.REWARDS] = (min_max_reward + current_batch[DISCRIMINATOR_REWARDS])
        ...
        if ORPO_TRAINING or ORPO_DISC_TRAINING:
            ...
            #print("episode_reward_with_discriminator_mean", np.mean(episode_rewards_with_discriminator))
        elif MAX_MIN:
            info[self.config.current_policy_id][
                "episode_worse_reward_with_discriminator_mean"
            ] = np.mean(episode_rewards_with_discriminator)
            print("episode_worse_reward_with_discriminator_mean", np.mean(episode_rewards_with_discriminator))
        elif LINEAR_MAX_MIN:
            info[self.config.current_policy_id][
                "episode_worse_reward"
            ] = np.mean(episode_rewards_with_discriminator)
            print("episode_worse_reward", np.mean(episode_rewards_with_discriminator))
        ...
        if ORPO_DISC_TRAINING or LINEAR_MAX_MIN:
            current_batch_copy = current_batch.copy()
            
            worse_policy_discriminator_reward = np.clip(
                worse_policy_discriminator_reward,
                -self.config.discriminator_reward_clip,
                self.config.discriminator_reward_clip,
            )
            current_batch_copy[DISCRIMINATOR_REWARDS] = worse_policy_discriminator_reward

            current_batch_copy[SampleBatch.REWARDS] = (
                normalized_reward + current_batch_copy[DISCRIMINATOR_REWARDS]
            )
            assert np.all(np.isfinite(current_batch_copy[SampleBatch.REWARDS]))


            episode_batches_copy = current_batch_copy.split_by_episode()
            episode_rewards_with_discriminator_copy = []
            for i in range(len(episode_batches_copy)):
                episode_rewards_with_discriminator_copy.append(
                    np.sum(episode_batches_copy[i][SampleBatch.REWARDS])
                )

            info[self.config.current_policy_id][
                "episode_worse_reward_with_discriminator_mean"
            ] = np.mean(episode_rewards_with_discriminator_copy)
            print("episode_worse_reward_with_discriminator_mean", np.mean(episode_rewards_with_discriminator_copy))
    ...
    def training_step(self) -> ResultDict:
        ...
        policy = self.workers.local_worker().policy_map["safe_policy0"]
        optimizer = policy._optimizers[0]

        if ORPO_DISC_TRAINING or LINEAR_MAX_MIN or MAX_MIN:
            original_lr = optimizer.param_groups[0]["lr"]

            #print(f"Original LR: {original_lr}")

            # Change learning rate, traffic: 5e-3, 200 step; pandemic: 5e-4, 10.  glucose: 1e-2, 20
            ...
            if self.config.simple_optimizer:
                for _ in range(200):
                    ...
                    #discriminator_results.append(train_results_safe_policy['safe_policy0']['learner_stats']['discriminator/loss'])
            else:
                train_results_safe_policy = multi_gpu_train_one_step(
                    self, safe_policy_train_batch
                )
            #plt.plot(range(len(discriminator_results)), discriminator_results)
            #plt.title('Discriminator Loss For one epoch',fontsize=17)
            #lt.xticks(fontsize=15)
            #plt.yticks(fontsize=15)
            #plt.savefig("discriminator_loss.jpg")   
            #exit()

            # Change back learning rate
        ...
        if self.config.train_discriminator_first:
            (
                ...
                train_batch, ORPO_TRAINING, ORPO_DISC_TRAINING, LINEAR_MAX_MIN, MAX_MIN
            )
