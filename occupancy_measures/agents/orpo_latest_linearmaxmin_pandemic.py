...
class ORPO(PPO):
    ...
    def caculate_linear_worse_reward(
        ...
        for i in range(len(episode_batches)):
            ...
            vel = np.array([entry["rew_breakdown"]["InfectionSummaryAbsoluteReward"] for entry in info_list[1:] if "rew_breakdown" in entry])
            accel = np.array([entry["rew_breakdown"]["LowerStageReward"] for entry in info_list[1:] if "rew_breakdown" in entry])
            headway = np.array([entry["rew_breakdown"]["SmoothStageChangesReward"] for entry in info_list[1:] if "rew_breakdown" in entry])
            vel_list.extend(vel)
        ...
        d = np.mean(np.array(normalized_reward)[:, np.newaxis]*B_weighted, axis=0)
        #print(d)
    ...
    def solve_lambda_(self, v, d, c, Q, r, init_lambda=None):
        """
    ...
    def augment_reward_with_discriminator_and_transfer_action_dist_inputs(
        ...
        for policy_id in self.config.safe_policy_ids:
            if policy_id in train_batch.policy_batches:
                ...
                for i in range(len(episode_batches_safe)):
                    ...
                    safe_vel = np.array([entry["rew_breakdown"]["InfectionSummaryAbsoluteReward"] for entry in info_list[1:] if "rew_breakdown" in entry])
                    safe_accel = np.array([entry["rew_breakdown"]["LowerStageReward"] for entry in info_list[1:] if "rew_breakdown" in entry])
                    safe_headway = np.array([entry["rew_breakdown"]["SmoothStageChangesReward"] for entry in info_list[1:] if "rew_breakdown" in entry])
                ...
                current_vel_list = [entry["rew_breakdown"]["InfectionSummaryAbsoluteReward"]  for entry in current_info_batch if "rew_breakdown" in entry]
                current_accel_list = [entry["rew_breakdown"]["LowerStageReward"] for entry in current_info_batch if "rew_breakdown" in entry]
                current_headway_list = [entry["rew_breakdown"]["SmoothStageChangesReward"] for entry in current_info_batch if "rew_breakdown" in entry]
                ...
                Q += 1e-8 * np.eye(Q.shape[0])           # Stability
                print(Q)
                ...
                # solve lambda
                lambdas = self.solve_lambda_(v, d, c, Q, correlated_r)
                if lambdas is not None:
                    lambda1, lambda2, lambda3 = lambdas
                    print("λ₁, λ₂, λ₃ =", lambda1, lambda2, lambda3)
        ...
        current_batch[DISCRIMINATOR_REWARDS] = np.zeros_like(current_batch[SampleBatch.REWARDS])
        ...
        episode_batches = current_batch.split_by_episode()
        episode_worse_rewards = []
        #unclipped_episode_rewards_with_discriminator = []
        for i in range(len(episode_batches)):
            episode_worse_rewards.append(
        ...
        ] = np.mean(episode_worse_rewards)
        print(np.mean(episode_worse_rewards))
    ...
    def training_step(self) -> ResultDict:
        ...
        # We actually do not need to train the discriminator
        if self.config.simple_optimizer:
            policy = self.workers.local_worker().policy_map["safe_policy0"]
            optimizer = policy._optimizers[0]
            original_lr = optimizer.param_groups[0]["lr"]
            # Change learning rate, traffic: 5e-3, 200 step; pandemic: . 
